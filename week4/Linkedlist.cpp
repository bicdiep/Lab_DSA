...
void removePos(List*& L, int data, int pos)
{
    if (L == nullptr || L->p_head == nullptr || pos < 0) return;
    if (pos == 0) {
        NODE* temp = L->p_head;
        L->p_head = L->p_head->p_next;
        delete temp;
    ...
    while (prev != nullptr && prev->p_next != nullptr && i < pos - 1) {
        prev = prev->p_next;
        i++;
    }
    ...
    while (p != nullptr) {
        ...
        while (q->p_next != nullptr) {
            if (q->p_next->key == p->key) {
                NODE* temp = q->p_next;
                q->p_next = temp->p_next;

                if (temp == L->p_tail) L->p_tail = q;
